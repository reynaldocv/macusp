	...
	$va_add_to_set_link_info = caGetAddToSetInfo($this->request);
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				$vs_cache_key = md5($vs_table.$vn_id."info".$vb_refine);
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result_info')){
					print ExternalCache::fetch($vs_cache_key, 'browse_result_info');
				}else{
					...
					ExternalCache::save($vs_cache_key, $vs_result_output, 'browse_result_info');
