				...
				if(is_array($va_sorts = $this->getVar('sortBy')) && sizeof($va_sorts)) {
					...
					foreach($va_sorts as $vs_sort => $vs_sort_flds) {
						...
						if ($vs_current_sort === $vs_sort) {
							print "<li class='selectedSort redLink'>".$vs_text."</li>\n";
						} else {
							print "<li>".caNavLink($this->request, $vs_text, '', '*', '*', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'sort' => $vs_sort, '_advanced' => $vn_is_advanced ? 1 : 0))."</li>\n";
						}
...
?>
		...
		<div id="bViews" class='macuspViews'>
			<h5>
				<?php					
					if(is_array($va_views) && (sizeof($va_views) > 1)){
						print "<strong>"._t("Views")."</strong> : "; 

						foreach($va_views as $vs_view => $va_view_info) {
							if ($vs_current_view === $vs_view) {
								$_title = _t("$vs_view");
								print '<a href="#" ><span class="hover-tip glyphicon '.$va_view_icons[$vs_view]['icon'].' redGlyphicon" data-text="'.$_title.'"> </span></a> ';
							} else {
								$_title = _t("$vs_view");
								print caNavLink($this->request, '<span class="hover-tip glyphicon '.$va_view_icons[$vs_view]['icon'].'" data-text="'.$_title.'"> </span>', 'enabled', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key)).' ';
							}
						}
					}
...
<script type='text/javascript'> 
	...
	function fullprogessbar(value)
	{			
		var progress = document.getElementById('progressbar');
		progress.style.width = "100%"; 
		progress.innerHTML = "100%";
		//progress.style.visibility = "hidden"; 
		var size = document.getElementById("vn_result_size").value; 
		if (size <= 2000)
		{
