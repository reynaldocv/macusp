 ...
 */
	...
	$ops_view_prefix = $this->getVar('ops_view_prefix');
	//$menu_items = $this->getVar('reynaldocv');
	
	include("macusp-template/template.php");
...
<div class="row" style="clear:both;">
	<div class="refineCol <?php print ($vs_refine_col_class) ? $vs_refine_col_class : "col-sm-4 col-md-3 col-md-offset-1 col-lg-3 col-lg-offset-0"; ?>">
		
		<div id='bMorePanel'><!-- long lists of facets are loaded here --></div>
		<div id='bRefine' class='bRefine'>
			<a href='#' id='bRefineClose' onclick='jQuery("#bRefine").toggle(); return false;'><span class='glyphicon glyphicon-remove-circle'></span></a>
			<H4>Filter Results By: 
			</H4>
			<H4>
			
				<div id = "divTextFilter" class="inputTextFilter">
							
				</div>

			</H4>
			<div class="browseRemoveFacet">
			...
			if (sizeof($va_criteria) > 0) {
				...
				foreach($va_criteria as $va_criterion) {
					//print "<strong>".$va_criterion['facet'].':</strong>';
						...
						//print_r($va_criterion['id']);
						if ($va_criterion['id'] != 1064)
						{ 
...
?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				...
				//if(jQuery('#browseResultsContainer').height() > jQuery(window).height()){
					var offset = jQuery('#bRefine').height(jQuery(window).height() - 300).offset();   // 0px top + (2 * 15px padding) + 80 (fixed header) + 80 (fixed footer) = 190px
	...
	<div class='margintop10 <?php print ($vs_result_col_class) ? $vs_result_col_class : "col-sm-8 col-md-8 col-lg-8"; ?> '>
...
<?php
		...
		if (sizeof($va_criteria) > 0) {
			$i = 0;
			print "<div class='responsiveFacetCriteria'><div class='browseRemoveFacet'>";
			foreach($va_criteria as $va_criterion) {
				if ($va_criterion['id'] != 1064){ 
					#print "<strong>".$va_criterion['facet'].':</strong>';
					print caNavLink($this->request, '<button type="button" class=""><span class="glyphicon glyphicon-remove"></span>'.$va_criterion['value'].' &nbsp;</button>', 'browseRemoveFacet', '*', '*', '*', array('removeCriterion' => $va_criterion['facet_name'], 'removeID' => $va_criterion['id'], 'view' => $vs_current_view, 'key' => $vs_browse_key))." ";
				}
				$i++;
...
?>
		<span style="right:0xp;position:right">
		<?php
			...
			if(is_array($va_views) && (sizeof($va_views) > 1)){
				foreach($va_views as $vs_view => $va_view_info) {
					if ($vs_current_view === $vs_view) {
						...
						print caNavLink($this->request, '<span class="glyphicon '.$va_view_icons[$vs_view]['icon'].'"></span>', 'enabled', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key)).' ';
					}
				}
			}
...
<?php
	print $this->render("$ops_view_prefix/browse_panel_subview_html.php");
} //!ajax
?>
...
<script type='text/javascript'> 
	...
	{			
		...
		progress.innerHTML = String(parseInt($value)) + "%";
	}
	function fullprogessbar()
	{			
		var progress = document.getElementById('progressbar');
		...
		jQuery("#divTextFilter").html("<input id='textfilter' type='text' autocomplete='off'>");

		var input = document.getElementById("textfilter");
		var divs1 = document.querySelectorAll(".bResultList");
		...
		function myFunction(e) {
			...
			//var divs2 = document.querySelectorAll(".bResultItemCol");
			//alert(divs1);
			
			if (filter.length > 3){
				for (var i = 0; i < divs1.length; i++) {
					var a = divs1[i].innerText;			
					
					if (a) {
						if (a.toUpperCase().indexOf(filter) > -1) {
							divs1[i].style.display = "";
						} else {
