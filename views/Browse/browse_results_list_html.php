			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if  ($vs_table == 'ca_entities')
					{
						...
						$vs_surname 	= strToUpper($qr_res->getWithTemplate("<b>^{$vs_table}.preferred_labels.surname</b>"));

						if (strtolower($vs_current_sort) === "name")
						{	if (trim($vs_forename) === "")
							{
								$vs_label_link 	= $vs_surname;
							}
							else
							{
								$vs_label_link 	= "$vs_surname, $vs_forename"; 
							}
						}
						else
						{
							$vs_label_link 	= $qr_res->get("{$vs_table}.preferred_labels.displayname");
						}
					}
					else{
						$vs_label_link 	= $qr_res->get("{$vs_table}.preferred_labels"); 
					}

					
					$vs_label_detail_link 	= caDetailLink($this->request, $vs_label_link, '', $vs_table, $vn_id);
					$vs_thumbnail = "";
					$vs_type_placeholder = "";
					$vs_typecode = "";
